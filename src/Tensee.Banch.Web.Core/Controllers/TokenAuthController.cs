...
using Tensee.Banch.Authentication.TwoFactor.Google;
...
namespace Tensee.Banch.Web.Controllers
{
    ...
    public class TokenAuthController : BanchControllerBase
    {
        ...
        /// 微信小程序消息推送校验接口
        /// </summary>
        /// <param name="input">校验信息</param>
        /// <returns></returns>
        [DontWrapResult]//禁用返回结果封装  这样就只返回bool
        [HttpGet]
        public string CheckWxSignature(Dto.WxCheckSignatureDto input)
        {
            string token = "test321";
            string tmpStr = input.GetSHA1(token);
            tmpStr = tmpStr.ToLower();
            if (tmpStr == input.signature)//校验成功，则返回输入的echostring，否则返回其他任意即可
                tmpStr = input.echostr;
            return tmpStr;
        }
        ...
        [HttpPost]
        public async Task<AuthenticateResultModel> QYAuthenticate([FromBody] QYAuthenticateModel model)//[FromBody] QYAuthenticateModel model
        {

            var loginResult = await QYGetLoginResultAsync(
                model.NameOrEmail,
                GetTenancyNameOrNull()
            );
        ...
        private async Task<AbpLoginResult<Tenant, User>> QYGetLoginResultAsync(string usernameOrEmailAddress,   string tenancyName)
        {
            var loginResult = await _externalLoginManager.LoginAsyncInternal(usernameOrEmailAddress,  tenancyName);
...
#if EXTERNLOGIN
            var loginResult = await _externalLoginManager.LoginAsync(new UserLoginInfo(model.AuthProvider, externalUser.ProviderKey, model.AuthProvider), GetTenancyNameOrNull());
#else
