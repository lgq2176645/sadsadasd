...
using Tensee.Banch.Authorization.Users.Profile.Cache;
...
namespace Tensee.Banch.Authorization.Users.Profile
{
    [AbpAuthorize]
    public class ProfileAppService : BanchAppServiceBase, IProfileAppService
    {
        ...
        public async Task<CurrentUserProfileEditDto> GetCurrentUserProfileForEdit()
        {
            ...
            userProfileEditDto.QrCodeSetupImageUrl = user.GoogleAuthenticatorKey != null
                ? _googleTwoFactorAuthenticateService.GenerateSetupCode("Tensee.Banch",
        ...
        public async Task<UpdateGoogleAuthenticatorKeyOutput> UpdateGoogleAuthenticatorKey()
        {
            ...
            {
                QrCodeSetupImageUrl = _googleTwoFactorAuthenticateService.GenerateSetupCode("Tensee.Banch",
                    user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300).QrCodeSetupImageUrl
            };
        }
        ...
        public async Task UpdateProfilePicture(UpdateProfilePictureInput input)
        {
            ...
            //sw.Write(tempProfilePicturePath);
            //sw.Close();
            byte[] byteArray;
        ...
        {
            GetProfilePictureOutput model =  await GetProfilePictureByIdInternal(profilePictureId);
            //StreamWriter sw = new StreamWriter("D:\\2.txt");
            //sw.Write(model.ProfilePicture);
            //sw.Close();
            return model;
        }
        ...
        public async Task<string> GetProfileById(Guid profilePictureId)
        {
            var bytes = await GetProfilePictureByIdOrNull(profilePictureId);
            if (bytes == null)
            {
